...
- サーバは外部公開しない（`127.0.0.1`固定）。`config.json` で host を変えても `config.enforce_loopback` がループバック以外を 127.0.0.1 へ強制するため、設定ミスでも外部公開されない
### リポジトリ
...
### 実機フィードバック反映（🔴暴発・本文表示）
M端末実機で「🔴が暴発」「吹き出しが見づらい」を受けて修正:
- **🔴暴発の主因＝解決不能To→🔴ルールの広域発火**: EX→SMTP変換が広く失敗する環境で `to_unresolved` が常時立ち🔴が無意味化。→ `config.red_on_unresolved_to`（既定false）でゲートし、🔴は「実際にToに自分が居る」基準に戻した。
- **自分のEX DNでも自己照合**: `my_addresses()` にSMTPだけでなく自分のEX DNも収集し、recipients解決がDNのままでも `to_list` に残して突き合わせる。SMTP変換が失敗しても「自分がToか」を判定できる。
- **送信済み＝確実に自分発**: `from_sent` のメールは送信者解決の成否に関わらず `is_from_me=True`。返信済みが🔴に残るのを防ぐ。
- **本文は削除せず正規化のみ（`models.clean_body`）**: この職場は引用記号 `>` 付き行に回答本体を書く運用があり、引用除去は回答消失になる。改行正規化（CRLF→LF・3連空行圧縮）に留め、フロントで「最新／以前のメール」を折りたたみ表示（インライン回答も展開で見える）。

### 初回ロード最適化（§6/§8 を実装）
実機の初回が遅かったため以下を実装:
- **コールドスタートのウィンドウ化**: 初回は直近 `COLD_WINDOW_DAYS`（既定90日, 環境変数 `MAILTALK_COLD_DAYS`）だけ取り込み即「準備完了」、それより古い分は同じ同期スレッドで**裏でバックフィル**（`iter_messages(before=...)`）。2回目以降は `last_sync_time` 起点の差分のみ。
- **本文の遅延化**: 列挙中は重い `HTMLBody` を読まない（`body_html=""`）。吹き出しは `body_preview`（Body先頭500字）で表示。
- **UIの継続更新**: 準備完了後もUIは低頻度ポーリングを続け、バックフィルされた古い会話・新着を反映。
- **未対応（将来）**: 会話を開いた時のフル本文(HTMLBody)遅延取得、返信等のエンドポイントからのCOM呼び出しはuvicornスレッド＝同期スレッドと別apartmentのため、クロススレッドCOM対策（専用COMワーカ）が必要（実機で返信検証時に対応）。

---
## 14. 配布・デプロイ（確定：exe on M端末・単一マシン）

### 端末構成（実環境の制約）
会社には2台あり、当初の「Python＋Outlook同一マシン」前提が崩れていた:
- **M端末**: Outlookあり。**Pythonインタプリタは使えない**が、**未署名exeは起動可能**（確認済み）。**UIはこのM端末のブラウザで見る**。
- **S端末**: Pythonが使える別端末。**Outlookは無い**。
- A・Bはネットワーク接続。共有フォルダで読み書き可能。

### 確定した解
**アプリ全体をPyInstallerで単一exeに固め、M端末で実行する。** これにより
Python・pywin32(win32com)・FastAPI/uvicorn・staticがexe内に揃い、Outlookと
同一マシン(M端末)で動くため **win32comがそのまま使え、リアルタイム取得も返信
(ReplyAll→Display)も成立する**。`source.get_default_source()` はWindowsで
`Win32OutlookSource` を選ぶので、追加の分岐は不要。

> 検討して**不採用**にした案: 共有フォルダ経由のJSON受け渡し（M端末でPS抽出→S端末で消費）。リアルタイム性が無く返信もできないため却下。PowerShell常駐HTTP案は、exeが起動可能と判明したため保留（exeが将来弾かれた場合の代替）。

### 役割分担とフロー
```
このPC(mac) : 開発・純ロジック検証(Fake) → GitHub push
S端末       : git pull → PyInstallerでビルド（mac不可なのでビルド機はS端末）
              uv pip install -e ".[windows,build]" ; pyinstaller mailtalk.spec
共有フォルダ : dist/MailTalk.exe をM端末へ受け渡し
M端末       : MailTalk.exe をダブルクリック → 127.0.0.1:8765 が自動でブラウザに開く
...
- **返信**: M端末ではローカルCOMなので §5 の `open_reply_draft`（ReplyAll→Display、`.Send()`禁止）がそのまま機能する。

### 設定の外部化（config.json）
調整値は `config.py` 経由で exe隣の `config.json` から読む（無ければ起動時に既定生成、ユーザー編集は上書きしない）。`cold_window_days`/`backfill_old`/`active_threshold`/`host`/`port`/`open_browser`。サンプルは `config.sample.json`（live `config.json` は.gitignore）。

### 初回ロードの追加最適化（COM往復の削減）
Outlook COMはSTA（単一スレッド）で**並列化しても読み取りは直列化される**ため、速度は「往復回数を減らす」方向で対処:
- **EX→SMTP解決キャッシュ**: 同一人物(DN/Address)の解決を `Win32OutlookSource._smtp_cache` で使い回す。recipients解決のCOM往復がコールド取得の主コストのため効果大。
- **DBバッチコミット**: `upsert_email/upsert_conversation` に `commit=False`、`_COMMIT_EVERY` 件ごと＋最後にまとめて `commit()`。
- **ウィンドウ＋backfill**（§前述）と **HTMLBody非読込**（§前述）と併用。
